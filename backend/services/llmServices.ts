...
import { getHealthSignalsByUsername } from "./healthSignalServices";
import { saveWeeklyDigest } from "./weeklyDigestService";
import ollama from "ollama";
import { zodToJsonSchema } from "zod-to-json-schema";
import z from "zod";

export async function generateHealthSignal_langchain({
  currentEntry,
  contextWindow,
  windowSize,
}: {
...
export async function generateHealthSignal_ollama({
  currentEntry,
  contextWindow,
  windowSize,
}: {
  ...
  try {
    ...
    const response = await ollama.chat({
      ...
      format: z.toJSONSchema(SignalOutput),
      stream: false,
      options: {
        temperature: 1.0,
        top_p: 0.95,
    ...
    const signalOutput = SignalOutput.parse(
...
export async function ollama_WeeklyDigest({
  rangeStart,
  rangeEnd,
  familyMember,
}: {
