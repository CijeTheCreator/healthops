...
export function ConversationView({
  ...
  return (
    <div
      className={`mx-auto flex w-full max-w-[980px] flex-col gap-6 flex-1 pb-10 ${isDraftOnly ? "justify-end" : ""}`}
    >
      {hasPrompt ? (
        <UserMessage
          title={"User"}
      ...
      {displayedRun ? (
        <>
          <ThoughtsPanel
            ...
            running={displayedRun.status === "running"}
          />
          <AssistantMessage
            run={displayedRun}
...
function AssistantMessage({
  ...
  const [expanded, setExpanded] = useState(false);
  const visibleMarkdown = expanded
    ? markdownContent
    : truncateAtBoundary(markdownContent, FINAL_PREVIEW_LIMIT).text;
  const isTruncated = markdownContent.length > visibleMarkdown.length;
  const hasPartialContent = Boolean(
    run.finalText.trim() || run.thoughtsText.trim(),
  );
    ...
    <section className="w-full max-w-[960px] self-center rounded-[4px] border border-[#646262] bg-[#1a1818]">
      <div className="flex items-center justify-between border-b border-[#302c2c] px-4 py-3 text-[13px] text-brand-text-muted">
        <div className="flex items-center gap-3">
          ...
          <span>{formatTime(run.completedAt ?? run.startedAt, "en")}</span>
          {status === "running" ? (
            <span className="text-brand-warning tabular-nums">
              {"Streaming..."} {elapsedLabel}
            </span>
          ) : elapsedLabel ? (
            <span className="text-brand-mid tabular-nums">{elapsedLabel}</span>
          ) : null}
        </div>
        <MessageMenu markdownContent={markdownContent} />
      </div>
      ...
      <div className="flex flex-col gap-4 px-4 py-4">
        ...
        <div className="markdown-body prose prose-invert max-w-none text-[15px] leading-[1.7] text-brand-light">
          {visibleMarkdown ? (
            <>
              ...
              {isTruncated ? (
                <div className="mt-4 rounded-[4px] border border-[#302c2c] bg-[#161515] px-4 py-3 text-[13px] text-brand-mid">
                  Long response is previewed to keep the page responsive.
                  <button
                    type="button"
                    onClick={() => setExpanded(true)}
            ...
            <div className="text-brand-mid">{"Waiting for a response."}</div>
          )}
        </div>
...
function ThoughtsPanel({
  ...
  const visibleThoughts = showFullThoughts
    ? thoughtsText
    : truncateAtBoundary(thoughtsText, THOUGHTS_PREVIEW_LIMIT).text;
          ...
          hasThoughts ? (
            <pre className="max-h-[280px] overflow-auto whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-mono text-[13px] leading-relaxed text-brand-mid">
              ...
              {thoughtsTruncated ? (
                ...
                thoughtsText.length > THOUGHTS_PREVIEW_LIMIT ? (
                <>
                  {"\n\n"}
                  <button
                    type="button"
            ...
            <div className="text-[13px] font-bold text-brand-light">
              {running
                ? "Waiting for model thoughts."
                : "No model thoughts received."}
            </div>
          )
        ) : (
          <div className="text-[13px] font-bold text-brand-light">
...
function MessageMenu({ markdownContent }: { markdownContent: string }) {
      ...
      {open ? (
        <div className="absolute right-0 top-8 z-20 w-48 rounded-[6px] border border-[#302c2c] bg-[#1f1d1d] py-1 shadow-2xl">
          ...
          <MenuItem
            ...
            onClick={() =>
              runAndClose(() => navigator.clipboard.writeText(markdownContent))
            }
          />
        </div>
      ) : null}
