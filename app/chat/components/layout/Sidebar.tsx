    ...
    <div className="w-[260px] flex-shrink-0 flex flex-col border-r border-[#646262] bg-[#1a1818] h-full overflow-y-auto">
        ...
        <div className="px-4 mb-2">
          ...
          <div className="space-y-1">
            {history.length > 0 ? (
              ...
              <div className="text-[13px] text-brand-mid px-2 py-1">
                {t("sidebar.noRuns")}
              </div>
            )}
          </div>
      ...
      <div className="p-4 border-t border-[#646262] text-[14px] space-y-3">
        ...
        {providerStatus.error ? (
          <p className="text-[12px] leading-relaxed text-brand-mid">
            {providerStatus.error}
          </p>
        ) : null}
      </div>
...
function TokenUsageBar({
  ...
  const percent = hasLimit
    ? Math.min(Math.round((tokens / tokenLimit) * 100), 100)
    : 0;
...
function HistoryItem({
  ...
  const hasPartialContent = Boolean(
    item.finalText.trim() || item.thoughtsText.trim(),
  );
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <span className="truncate text-[14px] text-brand-light">
            {runLabel(item.status, t)}
          </span>
