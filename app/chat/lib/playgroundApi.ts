...
export async function runPlayground(
  ...
  // Here we work on the dates first
  const requestPlayground: any = playground;
  requestPlayground["dateEnd"] = parseDate(playground["dateEnd"]);
  requestPlayground["dateStart"] = parseDate(playground["dateStart"]);

  const response = await fetch(`${ENDPOINT_URL}/api/run`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ playground: requestPlayground }),
    signal,
  });
